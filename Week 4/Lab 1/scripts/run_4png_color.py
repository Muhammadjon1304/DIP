...
# Load image in color
image_path = '/Users/muhammadjonparpiyev/Documents/DIP/Week 4/Lab 1/4.png'
I_input_bgr = cv2.imread(image_path, cv2.IMREAD_COLOR)
I_input = cv2.cvtColor(I_input_bgr, cv2.COLOR_BGR2RGB)

# Enhance color saturation for better visibility
hsv = cv2.cvtColor(I_input_bgr, cv2.COLOR_BGR2HSV).astype(np.float32)
hsv[:,:,1] = hsv[:,:,1] * 1.5  # Increase saturation by 1.5x
hsv[:,:,1] = np.clip(hsv[:,:,1], 0, 255)
hsv = hsv.astype(np.uint8)
I_input_enhanced = cv2.cvtColor(hsv, cv2.COLOR_HSV2RGB)

print('='*70)
print('TASK 1: GAMMA CORRECTION ON 4.PNG (COLOR)')
print('='*70)
print(f'\nOriginal Image Statistics (4.png):')
print(f'  Shape: {I_input_enhanced.shape}')
print(f'  Min: {I_input_enhanced.min()}, Max: {I_input_enhanced.max()}')
print(f'  Mean: {I_input_enhanced.mean():.2f}, Std: {I_input_enhanced.std():.2f}')
...
axes[0].imshow(I_input_enhanced)
...
for idx, gamma in enumerate(gamma_values, 1):
    I_gamma = gamma_correction(I_input_enhanced, c, gamma)
...
plt.tight_layout()
plt.savefig('/Users/muhammadjonparpiyev/Documents/DIP/Week 4/Lab 1/4_gamma_comparison_color.png', dpi=150, bbox_inches='tight')
print('\n✓ Gamma comparison image saved')

# Task 1 Part 2
best_gamma = 0.67
I_input_corrected = gamma_correction(I_input_enhanced, c, best_gamma)

print(f'\nOptimal Gamma: γ = {best_gamma}')
print(f'  Original: min={I_input_enhanced.min()}, max={I_input_enhanced.max()}, mean={I_input_enhanced.mean():.2f}')
print(f'  Corrected: min={I_input_corrected.min()}, max={I_input_corrected.max()}, mean={I_input_corrected.mean():.2f}')
print(f'\nReasoning:')
...
axes[0].imshow(I_input_enhanced)
axes[0].set_title('Original 4.png (Enhanced Color)', fontsize=13, fontweight='bold')
axes[0].axis('off')
axes[1].imshow(I_input_corrected)
axes[1].set_title(f'Gamma Corrected (γ = {best_gamma})', fontsize=13, fontweight='bold')
axes[1].axis('off')

plt.tight_layout()
plt.savefig('/Users/muhammadjonparpiyev/Documents/DIP/Week 4/Lab 1/4_gamma_result_color.png', dpi=150, bbox_inches='tight')
print('✓ Gamma result image saved')

I_input_corrected_bgr = cv2.cvtColor(I_input_corrected, cv2.COLOR_RGB2BGR)
cv2.imwrite('/Users/muhammadjonparpiyev/Documents/DIP/Week 4/Lab 1/4_gamma_corrected_color.jpg', I_input_corrected_bgr)
print('✓ Gamma corrected image saved')

# Task 2: Contrast Stretching
print('\n' + '='*70)
print('TASK 2: CONTRAST STRETCHING ON 4.PNG (COLOR)')
print('='*70)

# Part 1: Non-normalized
print(f'\nPART 1: Non-normalized Contrast Stretching (smax=255, smin=0)')
I_stretched_255_0 = contrast_stretching(I_input_enhanced.astype('float32'), s_min=0, s_max=255)
print(f'  Original: range={I_input_enhanced.min()}-{I_input_enhanced.max()}, mean={I_input_enhanced.mean():.2f}')
...
I_normalized = I_input_enhanced.astype('float32') / 255
...
for idx, (smax, smin) in enumerate(param_combinations, 1):
    I_stretched = contrast_stretching(I_input_enhanced.astype('float32'), s_min=smin, s_max=smax)
...
plt.tight_layout()
plt.savefig('/Users/muhammadjonparpiyev/Documents/DIP/Week 4/Lab 1/4_contrast_stretching_comparison_color.png', dpi=150, bbox_inches='tight')
...
print('\n' + '='*70)
print('ALL TASKS COMPLETED SUCCESSFULLY (4.PNG - COLOR ENHANCED)')
print('='*70)
